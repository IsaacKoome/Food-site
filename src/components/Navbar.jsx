...
const Navbar = () => {
  ...
  return (
    ...
    {/* Side draw menu */}
    <div className={nav ? 'fixed top-0 left-0 w-[200px] h-screen bg-white z-10 duration-300' :
        'fixed top-0 left-[-100%] w-[200px] h-screen bg-white z-10 duration-300' }>
      <AiOutlineClose
       onClick={() => setNav(!nav)}
      size={25} className='absolute right-4 top-4 cursor-pointer' />
      <h2 className='text-xl p-3'>Best <span className='font-bold'>Eats</span></h2>
      <nav>
        <ul className='flex flex-col p-4 text-gray-800'>
          ...
          <li className='text-xl py-3 flex'><FaUserFriends size={25} className='mr-4' />Invite friends</li>
          </ul>
      </nav>
    </div>
    </div>
